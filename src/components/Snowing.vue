<template>
  <div
    ref="wrap"
    class="snow-wrap background"
    :class="{ 'events-all': interaction, hide: hide }"
    v-if="internal_show"
  >
    <img
  ...
  methods: {
    ...
    reset: function (flake) {
      if (this.windPower == false || this.windPower == 0) {
        ...
        var xarray, yarray, allarray, selected_array;
        if (this.windPower > 0) {
          xarray = Array(Math.floor(Math.random() * this.canvas.width), 0);
